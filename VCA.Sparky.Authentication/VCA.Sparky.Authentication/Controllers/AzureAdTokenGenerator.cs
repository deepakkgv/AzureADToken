...
namespace VCA.Sparky.Authentication.Controllers
{
    ...
    public class AzureAdTokenGenerator : ControllerBase
    {
        ...
        public async Task<ActionResult> Login(
            ...
            [FromForm] string grant_type = "password")
        {
            ...
            using (var httpClient = new HttpClient())
            {
                ...
                if (grantType == "password")
                {
                    ...
                    requestParams = new PasswordTokenRequest
                    {
                        ...
                        client_id = clientId,
                        resource = resource,
                        username = credentials.username != null ? credentials.username : string.Empty,
                        password = credentials.password != null ? credentials.password : string.Empty
                    };
                }
                ...
                var requestData = requestParams != null
    ? requestParams.GetType().GetProperties()
    ...
    : new Dictionary<string, string>();
        ...
        private LoginCredentials ExtractBasicAuthCredentials(string authorizationHeader)
        {
            ...
            string pattern = "(?i)vcaantech\\\\"; // (?i) makes the regex case-insensitive

             username = Regex.Replace(username, pattern, "");
            if (!username.EndsWith("@vca.com", StringComparison.OrdinalIgnoreCase))
            {
                username += "@vca.com";
            }
